...
// Common copy functions for pickplace_msgs/PickBox
...
namespace PickBox {
  ...
  //----------------------------------------------------------------------------
  MDArray_T get_arr_pickplace_msgs_Object_var_arr(MDFactory_T& factory, const pickplace_msgs::PickBox::_objects_type& val) {
    auto _objectsoutArray = factory.createStructArray({1,val.size()},{"grasping_poses","type","id"});
    for (size_t idx = 0; idx < val.size(); ++idx){
    // _pickplace_msgs_Object_var_arr.grasping_poses
    _objectsoutArray[idx]["grasping_poses"] = get_arr_pickplace_msgs_Grasp_var_arr(factory, val[idx].grasping_poses);
    // _pickplace_msgs_Object_var_arr.type
  ...
  //----------------------------------------------------------------------------
  PICKPLACE_MSGS_EXPORT void copy_from_arr(boost::shared_ptr<pickplace_msgs::PickBox>& msg, const matlab::data::StructArray arr) {
    try {
        //objects
  ...
  //----------------------------------------------------------------------------
  PICKPLACE_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const pickplace_msgs::PickBox>& msg) {
    auto outArray = factory.createStructArray({1,1},{"objects","name","approach_pose"});
    // objects
    outArray[0]["objects"] = get_arr_pickplace_msgs_Object_var_arr(factory, msg->objects);
    // name
