...
// Service Client for manipulation_msgs/AddBoxRequest
...
MANIPULATION_MSGS_EXPORT void copy_from_arr(manipulation_msgs::AddBox::Request& request,
...
namespace manipulation_msgs {
namespace matlabhelper {
namespace AddBox_Response {
MANIPULATION_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory,
                                         const manipulation_msgs::AddBox::Response& response);
} // namespace AddBox_Response
} // namespace matlabhelper
} // namespace manipulation_msgs
class MANIPULATION_MSGS_EXPORT manipulation_msgs_srv_AddBox_client
    ...
    std::map<int64_t, manipulation_msgs::AddBox::Request> mRequestPool;
    manipulation_msgs::AddBox::Request mNextRequest;
  public:
    manipulation_msgs_srv_AddBox_client()
        : MATLABSvcClientInterface()
        , mNextRequest() {
    }
    virtual ~manipulation_msgs_srv_AddBox_client() {
    }
    virtual intptr_t createSvcClient(const std::string& srv_name,
        ...
        mClient = std::make_shared<ros::ServiceClient>(
            theNode->serviceClient<manipulation_msgs::AddBox>(srv_name));
    ...
    virtual bool send_request(void* sd,
        ...
        if (ret) {
            ...
            auto outArray =
                manipulation_msgs::matlabhelper::AddBox_Response::get_arr(mFactory, svc.response);
            if (sd != NULL) {
    ...
    virtual bool addRequestToPool(matlab::data::Array arr) {
        manipulation_msgs::AddBox::Request req;
        manipulation_msgs::matlabhelper::AddBox_Request::copy_from_arr(req, arr);
        mRequestPool.insert(
                {manipulation_msgs::matlabhelper::AddBox_Request::get_requestId_from_arr(arr), 
                         req});
        return true;
    }
...
CLASS_LOADER_REGISTER_CLASS(manipulation_msgs_srv_AddBox_client, MATLABSvcClientInterface)
#ifdef _MSC_VER
#pragma warning(pop)
#else
